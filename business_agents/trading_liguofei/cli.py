"""CLI for Liguofei trading agent."""
...
def run_command(no_llm: bool, run_date: str | None, data_dir: Path) -> None:
    try:
        result = LiguofeiTradingAgent(data_dir=data_dir, run_date=run_date).run(no_llm=no_llm)
        ...
        click.echo(f"trading-liguofei failed: {exc}", err=True)
