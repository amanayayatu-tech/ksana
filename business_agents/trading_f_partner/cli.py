"""CLI for FPartner trading agent."""
...
def run_command(no_llm: bool, run_date: str | None, data_dir: Path) -> None:
    try:
        result = FPartnerTradingAgent(data_dir=data_dir, run_date=run_date).run(no_llm=no_llm)
        ...
        click.echo(f"trading-f_partner failed: {exc}", err=True)
