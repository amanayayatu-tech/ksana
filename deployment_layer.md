...
status: draft
applies_to:
  - fengliu_reverse_odds (v0.4 → v0.5 待补丁)
  - wanmu_single_sided (v0.2 → v0.3 待补丁)
  - liguofei_zen_value (v0.4 → v0.5 待补丁)
not_applies_to:
  - research_system_event_bayesian (v0.3) - 研究上游 Agent 不持仓，不受本部署层约束
source_of_truth: 本文档抽取自冯柳 v0.4 / 万木 v0.2 / 李国飞 v0.4 三套已 frozen 方法论中的重合部署规则
related_files:
  - methodology_fengliu.md
  - methodology_wanmu_single_sided-3.md
  - methodology_liguofei_zen_value.md
...
在冯柳 v0.4 / 万木 v0.2 / 李国飞 v0.4 三套方法论文档中，**仓位、风控、流动性、权限、市值**等部署规则高度重合（详见 `methodology_comparison_table.md` 第二部分）。如果让三份文档各自维护这些规则，**改一个数字要改三处**，必然失同步。
...
三套交易 Agent 的 Layered Authority 仍然各自保留**自己特有的部署规则**（如李国飞的三段熔断、万木的 -6% 红线选项），具体冲突解决见本文档 Section 5。

研究上游 Agent（4.1 研究体系 v0.3）**不受本文档约束**——它不持仓不交易，没有部署层。
...
| 目标持仓数 | 5-8 只 | 万木 + 李国飞一致；冯柳未明确，按此采用 |
...
| 满仓门槛 | 仅在四阶段全 ✓ 且贝叶斯置信度 ≥ 80% 时允许 | 万木 + 李国飞一致 |
...
| **退出锁**：单票持仓 / 日均成交额 | ≤ 0.5% | ≤ 0.3% | 万木 + 李国飞一致 |
| **压力退出锁**：成交萎缩 70% 情景下退出天数 | ≤ 3 个交易日 | ≤ 2 个交易日 | 万木 + 李国飞一致 |
| **可研究性锁**：近 12 个月主流卖方研报覆盖 | ≥ 3 份 | ≥ 3 份 | 三套 100% 一致 |

### 3.3 流动性动态校准（硬规则）

> **采用李国飞 v0.4 的动态校准设计作为系统主版本。**
...
| 例外 | 仅在用户明确把该标的改为长期主仓且重新批准风控豁免时才能绕过 | 冯柳明确，万木/李国飞默认 |
...
### 4.2 组合回撤熔断（采用李国飞三段熔断作为系统主版本）

> **决策**：三套对组合回撤的处理差异最大（冯柳无、万木 -6% 单档、李国飞 -8/-12/-15 三段）。  
> **采用李国飞的三段熔断作为系统主版本**。  
> 工程理由：三段比单档更精细，能在不同回撤阶段差异化响应；而冯柳"不设组合红线"在工程上必须有兜底。
...
| 参数 | 数值 | 来源 |
|---|---|---|
| 年换手率上限 | **150%** | 冯柳 v0.4 明确；万木/李国飞未明确，按此采用 |
...
| 做空 | ❌ 不允许 | 100% 一致 |
| 做空对冲 | ❌ 不允许 | 100% 一致 |
| 衍生品（期权/期货/CFD 等）| ❌ 不允许 | 万木 + 李国飞明确禁止；冯柳隐含禁止 |
...
| Crypto | 不交易 | 仅 4.1 研究体系可研究，不进交易候选池 |

---

## 7. A 股案例处理规则（硬规则）

> 万木 v0.2 和李国飞 v0.4 都明确规定 A 股案例处理。本部署层统一为系统规则。
...
**冯柳 v0.4 的特殊处理**：冯柳不使用四重安全边际框架，它用 `odds_score` / `probability_score` / 关注度购买度错配代替。冯柳 Agent 的输出可以**不填四重安全边际字段**，但必须填 `odds_score / probability_score / dislocation_score`。Chairman 汇总时需把这两套框架做映射（详见明日 Chairman 设计）。

### 8.2 FCF 负值成长股替代门槛（不抽取，保留各方法论自己版本）

> **决策**：万木和李国飞对 SaaS / 生物科技的替代门槛细节不同（rNPV/EV：1.0 vs 1.5）。这是**真实的方法论 DNA 差异**，不是抄重的部署规则。  
> **本部署层不抽取**，保留各方法论自己版本。

引用：
- 万木的 SaaS / 生物科技替代门槛见 `methodology_wanmu_single_sided-3.md` Gate 7 `negative_fcf_alternative_thresholds`
- 李国飞的 SaaS / 平台型 / 生物科技替代门槛见 `methodology_liguofei_zen_value.md` Section 2 `成长股例外规则`
...
| 日内 / 隔夜交易 | 仅作风控观察，不作为主动交易目标 | 冯柳明确 |
| 季度级以上持仓 | 不是默认状态，必须重证三率/安全边际 | 万木明确 |

### 9.2 不抽取部分（属于方法论 DNA）

- 冯柳的"时间不值钱"哲学
- 李国飞的"5-10 年目标持有期 vs 时间盒 4 周/3 月/6 月"两层架构
...
### 10.3 `upstream_research_signals[]`（新增 - 与 4.1 研究体系对接）

```yaml
upstream_research_signals:
  - research_signal_id:                     # 引用 4.1 研究体系的信号 ID
    signal_summary:
...
| Agent | 保留的特殊规则 | 简述 |
|---|---|---|
| 冯柳 v0.5 | 无（v0.4 的 R1-R4 全部通用化）| 引用本文档 Section 11 |
| 万木 v0.3 | R5 | 核心问题 > 3 个 → `abstain`（来自原话）|
| 李国飞 v0.5 | R5 / R6 / R7 | R5 时间盒 vs 长期持有 / R6 心态好 vs 强制止损 + false_stop_loss_review / R7 95% vs 70% 双门槛 |
...
- [ ] **黄/橙/红熔断在牛市的修正**：李国飞设计的 -8/-12/-15 是基于年回撤容忍 15%。在强势牛市中，回撤可能短期触发但非系统性。是否需要"3 个交易日内自动恢复"机制？
- [ ] **48 小时冷却期的例外**：当 -7% 止损刚触发但 24 小时内出现 thesis 反转的硬证据，是否允许冷却期内重新建仓？默认禁止
- [ ] **流动性紧急校准的 VIX 阈值**：30 这个数字是李国飞 v0.4 工程推断，是否需要根据中美市场特性差异化（如港股用恒指波幅、美股用 VIX）？
- [ ] **A 股案例的"案例学习用"标注是否够**：当某个 A 股标的（如牧原）的研究价值很高时，是否允许通过 A+H 联动间接持有？默认禁止
- [ ] **年换手率 150% 的实操**：仅冯柳 v0.4 明确，是否真适合万木/李国飞？万木原本就限制频繁交易，可能更严
...
2. **Layered Authority 改写**：第一层 deployment_hard_rules 不再列具体数值，改为引用 `deployment_layer.md`，仅在本方法论有特殊补充时显式标注（如李国飞的三段熔断已是部署层主版本，不再单独列）
3. **Output Schema 新增三个字段块**：`deployment_compliance` / `authority_resolution`（增强）/ `upstream_research_signals[]`
4. **Layered Authority 新增 R8**：本 Agent 不自行调用 Perplexity，所有研究请求通过 pull_request 提交给 4.1 研究体系
...
- 保留 FCF 负值替代门槛中的 rNPV/EV ≥ 1.0（与李国飞的 1.5 不同，是方法论 DNA 差异）
...
- 三段熔断从"李国飞特有"改为"部署层主版本"，文档中只需说明"本方法论沿用 deployment_layer.md Section 4.2 三段熔断"

---

## 14. 修订历史

- v0.1 (2026-05-13)：首次抽取。基于冯柳 v0.4 / 万木 v0.2 / 李国飞 v0.4。
- v0.1.1 (2026-05-13)：小补丁。Section 12 加 DEC-018 完整 Open Question（账户规模变化时的 reduce 档位重算）。

## 15. 致谢

本部署层抽取建议最初来自 **Codex 在李国飞 v0.4 revision_summary 中的工程观察**："这些重叠是合理的，因为它们更像'同一账户、同一 PM、同一小资金规模'的部署层，而不是方法论 DNA。建议抽出独立的 `deployment_layer.md`，每套方法论文档只引用它。"
