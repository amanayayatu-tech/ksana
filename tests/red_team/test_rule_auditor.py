...
from tests.conftest import deployment, make_fengliu, make_liguofei, make_signal, make_wanmu
...
def test_ar_001_dual_gate_anomaly_detected():
    rec = make_liguofei("watch", dual_gate="anomaly_review_required")
...
def test_ar_002_deployment_failure_not_abstain():
    rec = make_wanmu("long", deployment_compliance=deployment(any_failure=True))
    findings = audit_rules([rec], [make_signal()], {})
    assert "AR-002" in rule_ids(findings)


def test_ar_003_abstain_reason_or_thesis_missing():
    rec = make_fengliu("abstain", abstain_reason="insufficient_data", thesis="太短")
    findings = audit_rules([rec], [make_signal()], {})
    assert "AR-003" in rule_ids(findings)


def test_ar_004_evidence_unverified_high_confidence():
    rec = make_wanmu("watch", confidence=80, evidence_unverified=True)
    findings = audit_rules([rec], [make_signal()], {})
